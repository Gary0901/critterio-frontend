...
const CHART_MIN_W = width - 72;  // card inner width
...
export default function PetDetailScreen({ navigation, route }: Props) {
  ...
  const saveCustomItem = () => {
    ...
    const cfg = CATEGORY_CONFIG_CARE[newItemCategory];
  ...
  const chartSvgW = Math.max(CHART_MIN_W, PAD_X * 2 + (weeklyLogs.length - 1) * POINT_SPACING);
  const chartPoints = (() => {
          ...
          <View style={[styles.sheet, { paddingBottom: insets.bottom + 24 }]}>
            ...
            <View style={styles.inputGroup}>
              ...
              <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
                {(Object.entries(CATEGORY_CONFIG_CARE) as [CareCategory, typeof CATEGORY_CONFIG_CARE[CareCategory]][]).map(([key, cfg]) => (
                  <TouchableOpacity
                    key={key}
